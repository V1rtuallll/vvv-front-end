...
每个 Git 分支是一套完整的网站版本。前端和后端是两个仓库，分支名按**大版本 + 前端样式后缀**对应：大版本名属于后端，前端在大版本名后加后缀，表示同一套后端上的另一种样式。

## 分支

- `main`：当前默认线上版本的代码基线，可以由管理员直接推送。
- 其他分支：另一套完整网站。新分支应从已有包含 `.github/workflows/` 的分支创建，确保可以 CI 和部署。
- **前端样式分支**：命名为 `大版本名_样式名`，例如 `V1rtualSS_sky`。前缀是它对应的**后端分支** `V1rtualSS`；`_` 之后的后缀只表示换了套样式，不单独占用后端分支。前端也可以直接用大版本名（如 `V1rtualSS`），表示该大版本的默认样式。
...
- 前后端独立发布。网站和 API 同时改变时，前端发布其样式分支（如 `V1rtualSS_sky`），后端发布对应的大版本分支（如 `V1rtualSS`）。
...
- 联动改动必须落在对应分支上：前端 `大版本名_样式名`（或 `大版本名`）对应后端 `大版本名`，例如 `V1rtualSS_sky` 对应 `V1rtualSS`。前后端仓库都要确认该分支已推送、CI 成功；只推送其中一个仓库或只看其中一端的 CI，不能视为联动版本可上线。
